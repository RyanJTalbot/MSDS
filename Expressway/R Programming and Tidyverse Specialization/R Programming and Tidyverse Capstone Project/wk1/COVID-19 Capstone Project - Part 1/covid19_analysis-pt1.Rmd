...
# Create a line plot for total cases and deaths
ggplot(daily_totals, aes(x = date)) +
  geom_line(aes(y = total_cases, color = "Total Cases"), linewidth = 1.2) +
  geom_line(aes(y = total_deaths, color = "Total Deaths"), linewidth = 1.2) +
  labs(x = "Date", y = "Count", color = "Metric") +
...
us_population_estimates_tidy <- us_population_estimates %>%
  pivot_longer(cols = -c(STNAME, CTYNAME), names_to = "year", values_to = "population") %>%
  mutate(year = str_extract(year, "\\d+")) %>%
  select(STNAME, CTYNAME, year, population) %>%
  mutate(FIPS = paste0(STNAME, CTYNAME)) %>%
  select(FIPS, year, population)

# Calculate the US population in 2020 and 2021
us_population_2020 <- sum(us_population_estimates_tidy$population[us_population_estimates_tidy$year == "2020"])
us_population_2021 <- sum(us_population_estimates_tidy$population[us_population_estimates_tidy$year == "2021"])

# Divide each statistic by the estimated population and multiply by 100,000
daily_totals_per_100k <- daily_totals %>%
  mutate(delta_deaths_1 = (delta_deaths_1 / us_population_2020) * 100000,
         delta_cases_1 = (delta_cases_1 / us_population_2020) * 100000,
         delta_deaths_7 = case_when(year(date) == 2020 ~ (delta_deaths_7 / us_population_2020) * 100000,
                                    year(date) == 2021 ~ (delta_deaths_7 / us_population_2021) * 100000),
         delta_cases_7 = case_when(year(date) == 2020 ~ (delta_cases_7 / us_population_2020) * 100000,
                                   year(date) == 2021 ~ (delta_cases_7 / us_population_2021) * 100000))

daily_totals_per_100k
...
ggplot(daily_totals_per_100k, aes(x = date)) +
  geom_point(aes(y = delta_cases_7, color = "Cases"), size = 3) +
  geom_point(aes(y = delta_deaths_7, color = "Deaths"), size = 3) +
